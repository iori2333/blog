import { ArticlePreview, Article } from '../models/article';
import { Tag } from '../models/tag';
import { get } from '../util/network';

export const fetchArticles = async (page = 0) => {
  const r = await get<ArticlePreview[]>('/api/articles', { page });
...
export const searchArticles = async (keyword: string, page = 0) => {
  const r = await get<ArticlePreview[]>('/api/search', { keyword, page });
  return r.data.sort((a, b) => b.timestamp - a.timestamp);
};

export const fetchArticle = async (id: string) => {
  const r = await get<Article>('/api/article', { id });
  return r.data;
};
